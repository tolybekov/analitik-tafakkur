...
with grade_titles as (
  select grade, array_agg(title order by title) as titles
  from public.tasks
  group by grade
),
demo_source as (
  select
    n,
    (5 + ((n - 1) % 5))::integer as grade,
    case
      when n % 2 = 1 then 65 + (1 + ((((n - 1) / 2) * 37) % 25))
      else 65 - (1 + ((((n - 1) / 2) * 37) % 25))
    end::integer as score,
    (timestamptz '2026-07-01 09:00:00+00' + (((n - 1) % 28) * interval '1 day') + ((n % 60) * interval '1 minute')) as submitted_at
  from generate_series(1, 402) as n
),
demo_records as (
  select
    'demo-' || lpad(n::text, 3, '0') as id,
    'AT-' || grade || upper(substr(md5('analitik-demo-' || n::text), 1, 5)) as anonymous_id,
    demo_source.grade,
    coalesce(
      grade_titles.titles[1 + ((n - 1) % greatest(coalesce(array_length(grade_titles.titles, 1), 1), 1))],
      'Analitik topshiriq'
    ) as task_title,
    score,
    submitted_at,
    submitted_at + interval '3 hours 7 minutes' as graded_at
  from demo_source
  left join grade_titles on grade_titles.grade = demo_source.grade
)
