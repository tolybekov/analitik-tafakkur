// Generated from the pasted 5th-9th grade task banks.
window.AT_EXTRA_TASKS = [
  {
...
// Demo public leaderboard records. Scores intentionally average 65 across 402 anonymous students.
window.AT_DEMO_SCOREBOARD = (function () {
  var total = 402;
  var tasks = Array.isArray(window.AT_EXTRA_TASKS) ? window.AT_EXTRA_TASKS : [];
  var titlesByGrade = tasks.reduce(function (groups, task) {
    groups[task.grade] = groups[task.grade] || [];
    groups[task.grade].push(task.title);
    return groups;
  }, {});
  ...
  function anonymousId(index, grade) {
    var seed = (index * 1103515245 + grade * 12345) >>> 0;
    var code = "";
    for (var i = 0; i < 5; i += 1) {
      seed = (seed * 1664525 + 1013904223) >>> 0;
      code += alphabet[seed % alphabet.length];
    }
    return "AT-" + grade + code;
  }

  return Array.from({ length: total }, function (_, index) {
    var number = index + 1;
    var grade = 5 + ((number - 1) % 5);
    var pairIndex = Math.floor(index / 2);
    var delta = 1 + ((pairIndex * 37) % 25);
    var score = 65 + (index % 2 === 0 ? delta : -delta);
    var titles = titlesByGrade[grade] || ["Analitik topshiriq"];
    var day = 1 + ((number - 1) % 28);
    var submittedMinute = number % 60;
    var gradedMinute = (submittedMinute + 7) % 60;

    return {
      id: "demo-" + pad(number, 3),
      anonymousId: anonymousId(number, grade),
      grade: grade,
      score: score,
      taskTitle: titles[(number - 1) % titles.length],
      submittedAt: "2026-07-" + pad(day, 2) + "T09:" + pad(submittedMinute, 2) + ":00Z",
      gradedAt: "2026-07-" + pad(day, 2) + "T12:" + pad(gradedMinute, 2) + ":00Z",
      isDemo: true
    };
  });
})();
